...
grid. Before we get started, we have to create a class to represent the perfect building.
...
to use a couple of operations. See the class description below.

class Building(south, west, width_WE, width_NS, height=10)

Returns a new Building instance with the South-West corner at [south, west] coordinates, the size is width_WE by
width_NS and the height of the building is height. "height" is a positive number with a default value of 10."""
...
class Building:
    ...
    def corners(self):
        return_dict = dict()
        return_dict["south-west"] = [self.south, self.west]
        return_dict["north-west"] = [self.south+self.width_NS, self.west]
        return_dict["south-east"] = [self.south, self.west+self.width_WE]
        return_dict["north-east"] = [self.south+self.width_NS, self.west + self.width_WE]
        return return_dict
    ...
    def __repr__(self):
        return (f'{self.__class__.__name__}('
                f'{self.south!r}, {self.west!r}, {self.width_WE!r}, {self.width_NS!r}, {self.height!r})')
